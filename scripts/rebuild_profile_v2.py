...
def load_reset_cutoff(connection: Any, user_id: int) -> int | None:
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT profile_reset_before_ts FROM user_profile WHERE user_id = %s",
            (user_id,),
        )
        row = cursor.fetchone()
    if row is None or row.get("profile_reset_before_ts") is None:
        return None
    return int(row["profile_reset_before_ts"])
...
def load_rebuild_events(
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            ...
            """,
            (user_id, list(_PROFILE_EVENT_TYPES), reset_cutoff, reset_cutoff),
        )
...
def replay_event(
    ...
    *,
    ...
    article_topic_ids = _stored_topic_ids(row)
    if not article_topic_ids and event.article_id is not None:
...
def rebuild_profiles(
    ...
    *,
    ...
    for target_user_id in target_user_ids:
        if dry_run:
            reset_cutoff = load_reset_cutoff(connection, target_user_id)
