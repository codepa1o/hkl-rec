...
def test_personas_returns_three_demo_personas(postgres_client, postgres_demo_user):
    r = postgres_client.get("/personas", params={"limit": 10})
    assert r.status_code == 200, r.text
    body = r.json()
    assert "items" in body
    user_ids = [item["user_id"] for item in body["items"]]
    assert postgres_demo_user in user_ids
...
def test_personas_respects_limit(postgres_client, postgres_demo_user):
    r = postgres_client.get("/personas", params={"limit": 2})
