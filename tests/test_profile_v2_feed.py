...
def test_positive_profile_v2_topic_adds_a_dedicated_recall_candidate(
    ...
    def fake_topic_recall(
        ...
        *,
        ...
        if topic_ids == [12]:
            return [{"news_id": "N501", "hot_score": 8.0}]
        return []

    monkeypatch.setattr(postgres, "load_news_ids_for_topics", fake_topic_recall)
    monkeypatch.setattr(postgres, "load_exploration_rows", lambda *args, **kwargs: [])
    ...
    candidates = repository._load_feed_candidates(
        ...
        page_size=1,
        user_id=7004,
        request_id="profile-v2-recall",
        use_als=False,
        as_of_ts=None,
        expected_catalog_fingerprint="catalog-fingerprint",
        excluded_news_ids=set(),
        session_id="feed-session",
        category=None,
    )

    assert candidates["N501"]["sources"] == {"profile_v2_topic"}
    assert candidates["N501"]["is_fallback"] is False
